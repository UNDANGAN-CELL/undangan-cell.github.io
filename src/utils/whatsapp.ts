...
export function buildCalculatorMessage(params: {
  ...
  let msg = `Halo Admin ${SITE_CONFIG.brandName}, saya telah menghitung estimasi di website:\n\n`;
  msg += `Produk / Tipe: ${params.productName}\n`;
  if (params.productCode) msg += `Kode Produk: ${params.productCode}\n`;
  msg += `Jumlah: ${params.quantity} pcs\n`;
  msg += `Harga Dasar: Rp ${params.basePrice.toLocaleString('id-ID')} / pcs\n`;

  if (params.addOns.length > 0) {
    msg += `Tambahan Layanan:\n${params.addOns.map(a => `• ${a}`).join('\n')}\n`;
  }
  ...
  if (params.notes) {
    msg += `Catatan: ${params.notes}\n`;
  }

  msg += `\nMohon konfirmasi ketersediaan dan rincian harga finalnya ya min. Terima kasih!`;
  return msg;
}

 ...
 */
const THUMBABLE_FOLDERS = ['images/undangan/', 'images/buku-yasin/'];

export function resolveThumbUrl(path: string): string {
  if (!path) return '';
  if (/^(https?:)?\/\//i.test(path) || path.startsWith('data:')) return path;
  const folder = THUMBABLE_FOLDERS.find((f) => path.includes(f) && !path.includes(`${f}thumb/`));
  const thumbPath = folder ? path.replace(folder, `${folder}thumb/`) : path;

...
export const GallerySection: React.FC<GallerySectionProps> = ({
  items = GALLERY_ITEMS,
  emptyTitle = 'Dokumentasi untuk kategori ini sedang kami siapkan',
  emptyDescription = 'Foto hasil produksi akan segera ditambahkan.',
}) => {
  ...
  return (
    <section id="galeri-section" className="py-8 sm:py-16 bg-[#faf7f2] border-t border-stone-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-8 space-y-2">
          ...
          <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl font-bold text-stone-900 tracking-tight">
            {items.length} Desain Hasil Cetak
          </h2>
          <p className="text-sm sm:text-base text-stone-600">
            Foto asli hasil cetak Rumah Ijo. Pilih kategori untuk melihat contoh undangan pernikahan, sunatan, aqiqah, ulang tahun, dan event.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-6">
          {categories.map((cat) => (
            ...
            >
              {cat}{cat !== 'Semua' && <span className="ml-1 opacity-70">({items.filter(item => item.category === cat).length})</span>}
            </button>
          ))}
        </div>

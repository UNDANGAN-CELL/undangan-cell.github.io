...
import { Search, SlidersHorizontal, X, ArrowUpDown, Tag, Sparkles, MessageCircle, PackageSearch, CheckCircle2 } from 'lucide-react';
import { Product, ProductCategory, CategoryInfo } from '../types';
import { CATEGORIES } from '../data/categories';
import { ProductCard } from './ProductCard';
import { buildCustomOrderMessage, getWhatsAppUrl } from '../utils/whatsapp';
...
interface ProductGridProps {
  ...
  title?: string;
  subtitle?: string;
  /** Ditampilkan saat hub ini belum punya produk sama sekali (bukan hasil filter/pencarian kosong). */
...
export const ProductGrid: React.FC<ProductGridProps> = ({
  ...
  title = 'Pilih Desain Sesuai Selera Anda',
  subtitle = 'Koleksi model cetak modern dan elegan. Bisa custom warna, teks doa, ornamen, dan penambahan foto.',
  comingSoonTitle = 'Katalog untuk kategori ini sedang disiapkan',
  ...
  const availableCategories = useMemo(() => categories.filter(cat => products.some(p => p.category === cat.id)), [products, categories]);
  ...
  return (
    <section id="katalog-section" className="py-8 sm:py-16 bg-[#f4f0e6] border-b border-stone-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {products.length === 0 ? (
          <div className="max-w-3xl mx-auto space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {categories.map((cat) => (
                <div key={cat.id} className="bg-[#faf8f4] rounded-2xl p-4 border border-stone-200/90 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-bold text-stone-800">{cat.title}</h3>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full border bg-amber-50 text-amber-800 border-amber-200 uppercase tracking-wide shrink-0">
                      Segera Hadir
                    </span>
                  </div>
                  {cat.infoPoints && (
            ...
            <div className="bg-[#faf8f4] rounded-2xl p-6 sm:p-8 text-center border border-amber-200/80 space-y-3">
              <div className="w-12 h-12 rounded-full bg-amber-50 text-amber-600 flex items-center justify-center mx-auto">
                <PackageSearch className="w-6 h-6" />
              </div>
              <div>
                <h3 className="text-sm sm:text-base font-bold text-stone-800">{comingSoonTitle}</h3>
                <p className="text-xs sm:text-sm text-stone-500 mt-1 leading-relaxed">{comingSoonDescription}</p>
              </div>
              <a
                href={getWhatsAppUrl(buildCustomOrderMessage({ designStatus: 'konsultasi', productType: title }))}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 py-3 px-5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold shadow-md transition min-h-[48px]"
              >
        ...
        <div className="bg-[#faf8f4] rounded-2xl p-3 sm:p-5 shadow-xs border border-stone-200/90 mb-4 sm:mb-8 space-y-2.5 sm:space-y-4">
        ...
        <div className="flex items-center justify-between text-xs text-stone-500 mb-2 sm:mb-4 px-1">
          <div>
        ...
        {/* Product Grid: 3 kolom di HP, 4 di desktop (biar ringkas & gak kepanjangan) */}

...
import { formatRupiah, calculateEstimate, CALCULATOR_ADDONS } from '../utils/calculator';
...
export const OrderFormModal: React.FC<OrderFormModalProps> = ({
  ...
  const estimateResult = calculateEstimate(product.price, quantity, selectedAddOns);
  ...
  const handleOrderSubmit = (e: React.FormEvent) => {
    ...
    const selectedAddOnNames = selectedAddOns.map(id => CALCULATOR_ADDONS.find(a => a.id === id)?.name).filter(Boolean) as string[];
        ...
        <form onSubmit={handleOrderSubmit} className="flex flex-col flex-1 min-h-0">
          ...
          <div className="overflow-y-auto p-4 sm:p-6 space-y-5 flex-1">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-stone-800 block">Tambahan Opsi (Opsional)</label>
              <p className="text-[11px] text-stone-500">Harga kartu di katalog adalah harga undangan saja. Plastik, label, amplop, dan finishing dihitung sebagai tambahan.</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                {CALCULATOR_ADDONS.map((addon) => {
                  const checked = selectedAddOns.includes(addon.id);
                  return (
                    <label key={addon.id} onClick={() => handleAddOnToggle(addon.id)} className={`flex items-start gap-2 p-3 rounded-xl border transition cursor-pointer select-none ${checked ? 'bg-emerald-50 border-emerald-300 text-emerald-900' : 'bg-white border-stone-200 text-stone-700 hover:bg-stone-50'}`}>
                      <input type="checkbox" checked={checked} onChange={() => {}} className="mt-0.5 rounded text-emerald-600 focus:ring-emerald-500" />
                      <span className="min-w-0"><strong className="block">{addon.name}</strong><span className="text-[10px] text-stone-500">+{formatRupiah(addon.pricePerUnit)} / pcs</span></span>
                    </label>
                  );
                })}
              </div>

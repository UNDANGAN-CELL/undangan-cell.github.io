...
import { MessageCircle, Sparkles, FileText, Paperclip, X } from 'lucide-react';
...
interface UploadDesignSectionProps {
  isOpen: boolean;
  onClose: () => void;
}

export const UploadDesignSection: React.FC<UploadDesignSectionProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      id="upload-desain-modal"
      className="fixed inset-0 z-[70] bg-stone-950/50 backdrop-blur-sm p-3 sm:p-6 flex items-center justify-center"
      role="dialog"
      aria-modal="true"
      aria-labelledby="upload-desain-title"
      onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div className="relative w-full max-w-3xl max-h-[92vh] overflow-y-auto rounded-2xl sm:rounded-3xl bg-[#f3efe6] shadow-2xl border border-stone-200">
        <button
          type="button"
          onClick={onClose}
          aria-label="Tutup pesanan custom"
          className="sticky top-3 float-right z-10 mr-3 sm:mr-5 mt-3 w-10 h-10 inline-flex items-center justify-center rounded-full bg-white/95 border border-stone-200 text-stone-600 hover:text-stone-900 shadow-sm cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        <section id="upload-desain-section" className="py-6 sm:py-8">
        ...
        <div className="max-w-3xl mx-auto bg-[#faf8f4] rounded-3xl border border-stone-200/90 p-6 sm:p-10 shadow-sm space-y-7">
          ...
          <div className="text-center space-y-2">
            ...
            <h2 id="upload-desain-title" className="font-serif text-xl sm:text-3xl font-bold text-stone-900 tracking-tight">

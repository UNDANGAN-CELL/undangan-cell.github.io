...
import { ArrowLeft, Layers, Camera, Calculator as CalcIcon } from 'lucide-react';
...
export type HubSection = 'katalog' | 'galeri' | 'kalkulator';
...
export const HubPage: React.FC<HubPageProps> = ({
  ...
  return (
    <div id={`hub-page-${hub}`}>
      ...
      <div className={`pt-32 sm:pt-28 pb-4 bg-[#f7f5ef] border-b border-stone-200/80 bg-gradient-to-b ${hubInfo.gradient}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <button
            id="hub-back-to-home-btn"
            onClick={onBack}
            className="inline-flex items-center justify-center gap-2 min-h-11 px-3.5 py-2.5 rounded-xl bg-white/90 border border-stone-200 text-xs sm:text-sm font-bold text-stone-700 hover:text-emerald-700 hover:border-emerald-300 shadow-sm mb-4 cursor-pointer transition touch-manipulation"
          >
          ...
          <div className="flex items-center gap-2.5">
            <span className={`inline-flex px-3 py-1 rounded-full text-xs font-extrabold uppercase tracking-wide border ${hubInfo.accentColor}`}>
              {hubInfo.navLabel}
            </span>
            <h1 className="font-serif text-xl sm:text-2xl font-extrabold text-stone-900">{hubInfo.title}</h1>
          </div>
      ...
      {section === 'katalog' && (
        <ProductGrid
          ...
          comingSoonDescription="Harga belum tersedia di website. Berikut gambaran varian yang kami layani — tanyakan detail & harga langsung ke admin."
        />
      )}

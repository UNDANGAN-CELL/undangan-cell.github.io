...
import { Calculator, MessageCircle, Info, Clock } from 'lucide-react';
import { CALCULATOR_ADDONS, calculateEstimate, formatRupiah } from '../utils/calculator';
import { buildCalculatorMessage, buildCustomOrderMessage, getWhatsAppUrl } from '../utils/whatsapp';
...
export const PriceCalculatorSection: React.FC<PriceCalculatorSectionProps> = ({
  products,
  comingSoonLabel = 'produk kategori ini',
}) => {
  // Sumber produk tunggal: products.ts (atau daftar produk hub yang diberikan). Produk tanpa harga tetap ditampilkan.
  const calculatorProducts = useMemo(() => products ?? PRODUCTS, [products]);
  ...
  if (calculatorProducts.length === 0) {
    return (
      <section id="kalkulator-section" className="py-8 sm:py-16 bg-[#faf7f2] border-t border-stone-200/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-xl mx-auto bg-[#faf8f4] rounded-2xl border border-amber-200/80 p-8 text-center space-y-4">
            <div className="w-14 h-14 rounded-full bg-amber-50 text-amber-600 flex items-center justify-center mx-auto">
              <Clock className="w-7 h-7" />
            </div>
            <div>
              <h3 className="text-base sm:text-lg font-bold text-stone-800">Kalkulator untuk {comingSoonLabel} segera hadir</h3>
              <p className="text-xs sm:text-sm text-stone-500 mt-1.5 leading-relaxed">
                Data harga belum tersedia di website, jadi estimasi belum bisa dihitung otomatis. Tanyakan langsung ke admin untuk penawaran harga.
              </p>
            </div>
            <a
              href={getWhatsAppUrl(buildCustomOrderMessage({ designStatus: 'konsultasi', productType: comingSoonLabel }))}
              target="_blank"
